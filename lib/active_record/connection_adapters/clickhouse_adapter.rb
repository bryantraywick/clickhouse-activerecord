...
module ActiveRecord
  ...
  module ConnectionAdapters
    ...
    class ClickhouseAdapter < AbstractAdapter
      ...
      def create_dictionary(dict_name, request_settings: {}, **options, &block)
        ...
        if options[:with_table]
          ...
          create_table(table_name, request_settings:, **options.merge(options: dictionary_options).except(:dictionary), &block)
        end
      end
      ...
      def drop_dictionary(dict_name, options = {})
        execute("DROP DICTIONARY#{' IF EXISTS' if options[:if_exists]} #{quote_table_name(dict_name)}")
